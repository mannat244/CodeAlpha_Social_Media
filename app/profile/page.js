...
import { Heart, Mail,  Users, UserRoundPlus, NotebookPen, Mailbox, HandHeart} from "lucide-react"; // Optional: if using Lucide icons
 ...
 const toggleFollow = async () => {
  ...
  try {
    const res = await fetch("/api/follow", {
      ...
      body: JSON.stringify({ userIdToFollow: profile.user._id }),
    });
    if (!res.ok) throw new Error("Failed to update follow status");
    await res.json(); // You can ignore the response here

    // Now refetch profile to get updated follow state and counts
    const profileRes = await fetch(`/api/profile?userId=${profile.user._id}`);
  ...
  if (!profile) return <div className="text-center mt-20">No profile data.</div>;
  ...
  try {
    ...
    const res = await fetch(userId ? `/api/profile?userId=${userId}` : "/api/profile");
    const data = await res.json();
    setProfile(data);
  } catch (err) {
          ...
          {!isOwnProfile && (
            ...
            >
              {followLoading ? (
                ...
                <>
                  <span className="text-sm"><UserRoundPlus /></span>
                  Follow
                </>
              )}
            </button>
    ...
    <div className="bg-gradient-to-r from-slate-800/90 to-slate-700/90 backdrop-blur-xl rounded-2xl border border-slate-600/50 shadow-2xl p-8">
      <div className="flex items-center gap-3 mb-8">
        <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg flex items-center justify-center">
          <span className="text-white text-sm font-bold"><NotebookPen /></span>
        </div>
        ...
        {enrichedPosts.length === 0 ? (
          <div className="text-center py-16">
            <div className="w-16 h-16 bg-slate-700/50 rounded-full flex items-center justify-center mx-auto mb-4">
              <span className="text-2xl"><Mailbox /></span>
            </div>
            ...
            >
              ...
              <div className="flex items-center justify-between border-t border-slate-600/30 pt-4">
                ...
                <div className="flex items-center gap-2 text-slate-500">
                  <span className="text-sm"><HandHeart /></span>
                  <span className="text-sm">
                    {post.likesCount === 1 ? '1 like' : `${post.likesCount || 0} likes`}
                  </span>
                </div>
              </div>
            </div>
